    ...
    <div class="wrapper wrapper-content animated fadeInRight">
        <div class="row">
                ...
                <div class="ibox float-e-margins">
                    ...
                    <div class="ibox-content">
                        ...
                        <div class="form-group">
                            <form action="{{action('Admin\CatgWebboardController@index')}}" method="get">
                        ...
                        <table class="table table-bordered">
                            ...
                            <tbody>
                                @foreach($channels as $channel)
                                    <tr>
                                        <td class="text-center">{{$channel->order}}</td>
                                        <td>{{$channel->name}}</td>
                                        <td class="text-center">{{count($channel->topics)}}</td>
                                        <td class="text-center">
                                            @if(count($channel->topics))
                                            ...
                                            @else
                                                <span data-toggle="tooltip" title="Delete">
                                                    <i class="fa fa-trash text-danger" style="cursor:pointer;" onclick="$(this).find('form').submit();">
                                                        <form action="{{action('Admin\CatgWebboardController@destroy', $channel->id)}}" method="POST" hidden>
                                            ...
                                            <span data-toggle="tooltip" title="Edit">
                                                <i class="fa fa-pencil-square-o text-success"  style="cursor:pointer;" data-toggle="modal" data-target="#updateCategory-{{$channel->id}}"></i>
                                            </span>

                                            <!-- Edit Category Name Modal Pop -->
                                            <div class="modal fade" id="updateCategory-{{$channel->id}}" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
                                                <div class="modal-dialog" role="document">
                                                    <form action="{{action('Admin\CatgWebboardController@update', $channel->id)}}" method="POST">
                                                        {!! method_field('put') !!}
                                                        {!! csrf_field() !!}
                                                                    ...
                                                                    <div class="col-sm-10">
                                                                        <select name="order" class="form-control">
                                                                            @for($i=1; $i<=$channels->count(); $i++)
                                                                                <option value="{{$i}}" @if($i === $channel->order) selected @endif>{{$i}}</option>
                                                                            @endfor
                                                                        </select>
                                                                    </div>
                                                                    <br><br>
            ...
            <div class="modal fade" id="createCategory" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
                <form action="{{action('Admin\CatgWebboardController@store')}}" method="POST">
                    {!! csrf_field() !!}
                    <div class="modal-dialog" role="document">
                        <div class="ibox float-e-margins">
                            <div class="ibox-title">
