    ...
    <div class="wrapper wrapper-content animated fadeInRight">
        <div class="row">
                ...
                <div class="ibox float-e-margins">
                    ...
                    <div class="ibox-content">
                        ...
                        <div class="form-group">
                            <form action="{{action('Admin\CatgAdsController@index')}}" method="get">
                        ...
                        <table class="table table-bordered">
                            ...
                            <tbody>
                                @foreach($adsCatgs as $ads)
                                    <tr>
                                        <td class="text-center">{{$ads->order}}</td>
                                        <td>{{$ads->name}}</td>
                                        <td class="text-center">{{count($ads->adsList)}}</td>
                                        <td class="text-center">
                                            @if(count($ads->adsList))
                                            ...
                                            @else
                                                <span data-toggle="tooltip" title="Delete">
                                                    <i class="fa fa-trash text-danger" style="cursor:pointer;" onclick="$(this).find('form').submit();">
                                                        <form action="{{action('Admin\CatgAdsController@destroy', $ads->id)}}" method="POST" hidden>
                                            ...
                                            <span data-toggle="tooltip" title="Edit">
                                                <i class="fa fa-pencil-square-o text-success"  style="cursor:pointer;" data-toggle="modal" data-target="#updateCategory-{{$ads->id}}"></i>
                                            </span>

                                            <!-- Edit Category Name Modal Pop -->
                                            <div class="modal fade" id="updateCategory-{{$ads->id}}" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
                                                <div class="modal-dialog" role="document">
                                                    <form action="{{action('Admin\CatgAdsController@update', $ads->id)}}" method="POST">
                                                        {!! method_field('put') !!}
                                                        {!! csrf_field() !!}
                                                                ...
                                                                <div class="form-group">
                                                                    ...
                                                                    <div class="col-sm-10">
                                                                        <select name="order" class="form-control">
                                                                            @for($i=1; $i<=$adsCatgs->count(); $i++)
                                                                                <option value="{{$i}}" @if($i === $ads->order) selected @endif>{{$i}}</option>
                                                                            @endfor
                                                                        </select>
                                                                    </div>
                                                                    <br><br>
                                                                    ...
                                                                    <div class="col-sm-10">
                                                                        <input name="name" type="text" class="form-control" value="{{$ads->name}}">
                                                                    </div>
                                                                </div>
                                                            </div>
            ...
            <div class="modal fade" id="createCategory" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
                <form action="{{action('Admin\CatgAdsController@store')}}" method="POST">
                    {!! csrf_field() !!}
                    <div class="modal-dialog" role="document">
                        <div class="ibox float-e-margins">
                            <div class="ibox-title">
